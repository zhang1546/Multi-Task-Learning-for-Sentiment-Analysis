...
class generate(nn.Module):
    def __init__(self, args):
        ...
        self.task_recognized = CNN_layers(args.num_channels, args.kernel_sizes, args.bert_dim)
        ...
        if args.bidirectional:
            ...
            self.fc_recognized = nn.Linear(sum(args.num_channels), args.task_num)

        else:
            self.fc_specific = nn.Linear(sum(args.num_channels)+args.enc_hid_size, args.output_size)
            self.fc_recognized = nn.Linear(sum(args.num_channels)+args.enc_hid_size, args.task_num)
        nn.init.xavier_normal_(self.fc_specific.weight)
        nn.init.xavier_normal_(self.fc_recognized.weight)
        self.pool = nn.AdaptiveMaxPool1d(1)

    def forward(self, input):
        emb = self.embedding(input["x"])
        recognized = self.task_recognized(emb.permute(0, 2, 1))
        specific = self.task_specific[input["task_id"]](emb.permute(0, 2, 1))

        target = self.fc_specific(specific)
        task = self.fc_recognized(recognized)
